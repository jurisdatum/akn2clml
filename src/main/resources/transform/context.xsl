...
<xsl:function name="local:clml-element-is-block" as="xs:boolean">
	<xsl:param name="name" as="xs:string" />
	<xsl:choose>
		<xsl:when test="$name = ('UnorderedList', 'Tabular', 'Figure', 'Formula', 'BlockAmendment', 'BlockText', 'Text')">
			<xsl:value-of select="true()" />
		</xsl:when>
		<xsl:otherwise>
			<xsl:value-of select="false()" />
...
<xsl:function name="local:get-block-wrapper" as="xs:string?">
	...
	<xsl:choose>
		...
		<xsl:when test="head($context) = ('EnactingText', 'BlockText', 'ListItem', 'FootnoteText', 'Commentary', 'td')">
